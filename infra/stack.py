...
def to_us_inference_profile_id(model: bedrock.FoundationModel) -> str:
    return "us." + model.model_arn.partition("/")[2]


def to_us_inference_profile_arn(model: bedrock.FoundationModel, account: str) -> str:
    arn_prefix, _, model_id = model.model_arn.partition("/")
    arn_prefix, _, resource = arn_prefix.partition("::")
    return f"{arn_prefix}:{account}:inference-profile/us.{model_id}"


def grant_invoke_us_inference_profile(
    model: bedrock.FoundationModel,
    grantee: LwaLambdaFunction,
    account: str,
    region: str,
) -> None:
    for p in [
        iam.PolicyStatement(
            ...
            resources=[model.model_arn.replace(region, "*")],
        ),
        iam.PolicyStatement(
            actions=[
            ...
            resources=[to_us_inference_profile_arn(model, account)],
        ),
    ]:
        grantee.function.add_to_role_policy(p)
...
class InterviewPrepStack(cdk.Stack):
    ...
    def __init__(
        ...
        *,
        ...
        # ---- BEDROCK MODELS (CR Inference Profiles) ----

        agent_model = bedrock_alpha.CrossRegionInferenceProfile.from_config(
            geo_region=bedrock_alpha.CrossRegionInferenceProfileRegion.US,
            model=bedrock_alpha.BedrockFoundationModel.ANTHROPIC_CLAUDE_HAIKU_4_5_V1_0,
        )

        # Wait for addition of Opus 4.6 in cdk-bedrock-alpha
        # subagent_model = bedrock_alpha.CrossRegionInferenceProfile.from_config(
        #     geo_region=bedrock_alpha.CrossRegionInferenceProfileRegion.US,
        #     model=bedrock_alpha.BedrockFoundationModel.from_cdk_foundation_model_id(
        #         bedrock.FoundationModelIdentifier("anthropic.claude-opus-4-6-v1")
        #     ),
        # )
        # Workaround
        subagent_model = bedrock.FoundationModel.from_foundation_model_id(
            self,
        ...
        tavily_secret = secretsmanager.Secret.from_secret_name_v2(
            self,
            "tavily-secret",
            "research-subagent/tavily/api-key",
        )
        ...
        if local_dev:
            ...
            _ = cdk.CfnOutput(
                ...
                value=textwrap.dedent(f"""
                    ...
                    AGENT_MODEL={agent_model.inference_profile_id}
                    RESEARCH_SUBAGENT_MODEL={to_us_inference_profile_id(subagent_model)}
            ...
            agent_model.grant_invoke(agent_lambda.function)
            ...
            subagent_lambda = LwaLambdaFunction(
                ...
                environment={
                    "RESEARCH_SUBAGENT_MODEL": to_us_inference_profile_id(
                        subagent_model
                    ),
                    "LOGFIRE_SECRET": logfire_secret.secret_name,
                    "TAVILY_SECRET": tavily_secret.secret_name,
            ...
            # Wait for addition of Opus 4.6 in cdk-bedrock-alpha (use Sonnet 4.5 for now)
            # subagent_model.grant_invoke(subagent_lambda.function)
            # Workaround
            grant_invoke_us_inference_profile(
                subagent_model, subagent_lambda, self.account, self.region
            )
            # End Workaround
